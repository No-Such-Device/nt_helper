...
7. **Confirm destructive actions**: Always confirm before `new` (clears preset) or `edit_preset` (replaces the complete preset). For a full backup or complete state read, page through `get_preset` by following `paging.next` exactly until `has_more` is false. Do not modify the source preset between pages. Each page's compact `edit_slot_arguments` can be passed directly to `edit_slot` when creating or restoring a preset.
